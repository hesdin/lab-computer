        ...
        <div class="container-fluid">
            ...
            <div class="card rounded p-4">
                <div class="card-header">
                    @php
                        $data = \App\Models\Dosen::where('id_dosen', auth()->user()->id)
                        ->join('matakuliah', 'dosen.id_matakuliah', 'matakuliah.id')
                        ->select('matakuliah.id', 'matakuliah.matakuliah', 'matakuliah.semester')->get();
                    @endphp
                    @if ($data->count() > 1)
                        <div class="d-flex">
                            <button class="btn btn-primary btn-sm mr-3" onclick="document.location.href = '/o/dashboard/slip'"><i class="fas fa-arrow-left"></i></button>
                            <div class="float-right">
                                <button class="btn btn-success btn-sm" {!! ($daftarSlip->count()) ? 'onclick="document.location.href = \''.Request::url().'/printexcel\'"' : 'disabled' !!}><i class="fas fa-file-export mr-0"></i> Excel</button>
                            </div>&nbsp;&nbsp;
                            <h6 class="ml-auto">Jumlah Slip Praktikum: {{ $daftarSlip->count() }}</h6>
                        </div>
                    @endif
                </div>
                <div class="card-body">
